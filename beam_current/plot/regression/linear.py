...
# -*- coding: utf-8 -*-
"""
加载已保存的线性回归模型，无需重新训练，直接生成预测分析图。

运行方式（从 beam_current/ 目录执行）：
    python plot/regression/linear.py
    python plot/regression/linear.py --model result/linear/20260309_102805/linear_model_20260309_102805.pkl
"""
...
SPLIT_MODE = "time"   # 须与训练时一致
TEST_SIZE  = 0.2      # 须与训练时一致
# ══════════════════════════════════════════════════════════


def find_latest_model(result_root: str = "result/linear") -> str:
    """在 result/linear/ 下按目录名排序，找到最新的 linear_model_*.pkl"""
    pattern = os.path.join(result_root, "*", "linear_model_*.pkl")
    candidates = sorted(glob.glob(pattern))
    if not candidates:
        raise FileNotFoundError(
            f"在 {result_root} 下未找到任何 linear_model_*.pkl，请先运行 src/linear.py 训练模型。"
        )
    return candidates[-1]
...
def plot_results(y_train_true, y_train_pred, y_test_true, y_test_pred,
    ...
    out_path = os.path.join(save_dir, f"linear_regression_analysis_{timestamp}.png")
...
def main():
    parser = argparse.ArgumentParser(description="加载线性回归模型并绘图（无需重新训练）")

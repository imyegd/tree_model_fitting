...
def ensure_result_dir():
    ...
    result_dir = f"./result/tree_model/{timestamp}"
...
def train_gradient_boosting(X_train, y_train, n_estimators=100, max_depth=3, learning_rate=0.1,
                           subsample=0.8, min_samples_split=2, min_samples_leaf=1, random_state=42):
    """
    训练梯度提升树模型
    ...
    Returns:
        GradientBoostingRegressor: 训练好的模型
    """
    print("开始训练梯度提升树模型...")
...
def train_and_evaluate_model(model_type, X_train, X_test, y_train, y_test, feature_columns, result_dir, 
                            use_tuning=True, split_mode='time', test_size=0.2, random_state=None):
    """
    训练和评估指定类型的模型
...
def main():
    ...
    try:
        ...
        # # 定义要训练的模型类型
        models_to_train = ['decision_tree', 'random_forest', 'gradient_boosting']
        # models_to_train = []

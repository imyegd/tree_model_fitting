...
from common.utils import evaluate_model_performance 
...
#     # 绘制实际值和预测值的对比图
#     plot_prediction_comparison(y_test, y_pred, name)
...
def train_and_evaluate_model(model, X_train, X_test, y_train, y_test, name, use_scaler=False):
    ...
    plot_prediction_comparison(y_train, y_train_pred, y_test, y_test_pred, name)
    
    return results


def plot_prediction_comparison(y_train_true, y_train_pred, y_test_true, y_test_pred, model_name):
    """
    ...
    # 1. (Delta_X, Delta_Y) 2D坐标对比图 - 主要图表，占据更大的空间
    ax_main = plt.subplot(2, 3, (1, 4))  # 占据左侧两行
    # 训练集
    ...
    ax_main.set_xlabel('Delta_X', fontsize=12)
    ax_main.set_ylabel('Delta_Y', fontsize=12)
    ...
    # 保存图片
    result_dir = 'result/baseline_models'
    os.makedirs(result_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_path = os.path.join(result_dir, f"{model_name.replace(' ', '_')}_comparison_{timestamp}.png")
